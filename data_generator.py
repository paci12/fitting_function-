...
np.random.seed(42)
train_data_variables = np.random.uniform(0, 100, (train_num, variables_num))
np.random.seed(43)
val_data_variables = np.random.uniform(0, 100, (val_num, variables_num))
np.random.seed(44)
test_data_variables = np.random.uniform(0, 100, (test_num, variables_num))
...
def generate(mode='val',func=function,train_data_variables=train_data_variables):
    ...
    # Todo 修改变量
    df = pd.DataFrame(combined_array, columns=['x', 'y', 'result'])
